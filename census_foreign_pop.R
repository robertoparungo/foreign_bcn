...
census <- read_excel("Data/dissimilarity calculations.xlsx", 
                     sheet = "census dissimilarity", col_types = c("text", 
                                                                   "numeric", "numeric", "numeric", 
                                                                   "numeric", "numeric", "numeric", 
                                                                   "numeric", "numeric", "numeric"))

census2 <- census %>% 
  rename("CUSEC" = "Seccion")
...
spain$CUSEC <- as.numeric(as.double(spain$CUMUN))
census2$CUSEC <- as.numeric(as.character(census2$CUSEC))
...
#join data
census3 = left_join(spain, census2, by ="CUSEC")

#filter
filtered_census3 <- filter(census3, CUSEC %in% census2$CUSEC)

#plot

gg1 = ggplot() +
  geom_sf(data = filtered_census3, color = "transparent", size =0.1, aes(fill = foreignerovertotal)) +
  scale_fill_viridis(discrete=FALSE,
                     name="% Foreign population over total 
census population (hundreths)",
                     option="viridis",
                     direction=1) +
  theme(
...
#save
dir.create("maps")
ggsave(gg1, filename = "maps/census.png", width = 8.335, height = 6.946, dpi = 150)
